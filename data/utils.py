...
def build_ret_label(label_column):
    if pd.isnull(label_column) == True:
        return
    if label_column < -0.05:
        return "强烈不推荐"
    elif    label_column < -0.02 and label_column >= -0.05:
        return "不推荐"
    elif label_column >= -0.02 and label_column <= 0.02:
        return "一般"
    elif label_column > 0.02 and label_column <=0.05:
        return "推荐"
    else: 
...
def build_response(df,label_freq: str):
    """
    label_column is either 'week','month','year'
    """
    label_columns = {
        'week': 'ret_after_1_week',
        'month': 'ret_after_1_month',
        'year': 'ret_after_1_year'
    }

    df['response'] = df[label_columns[label_freq]].apply(build_ret_label)
    return df